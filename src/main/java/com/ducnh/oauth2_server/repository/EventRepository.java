...
public interface EventRepository extends CrudRepository<StravaEvent, String> {
    ...
    @Query(value = "SELECT x.athlete_id, x.event_id, x.team_id, e.event_name FROM strava_register x join strava_event e on e.id = x.event_id WHERE e.start_date <= NOW() AND e.end_date >= NOW() AND x.athlete_id = ?1", nativeQuery = true)
    List<Map<String, Object>> findCurrentEventByAthlete(Long athleteId);
}
